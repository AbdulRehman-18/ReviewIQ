...
import { ShieldCheck, Clock, CheckCircle, AlertTriangle, UploadCloud } from "lucide-react";
...
export default function ModerationPage() {
  ...
  const hasData = (data?.reviews?.items?.length ?? 0) > 0;
  ...
  const tabCount = (filter: FilterTab) =>
    filter === "all" ? queue.length : queue.filter((r) => r.flagReason === filter).length;
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Review AI-flagged items — sarcasm and ambiguous sentiment detected in your dataset.
          </p>
        </div>
        {stats.total > 0 && (
      ...
      {!hasData ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-muted-foreground">
          <UploadCloud className="w-12 h-12 opacity-25" />
          <p className="text-base font-medium">No data ingested yet</p>
          <p className="text-sm">
            Go to{" "}
            <button className="underline text-primary" onClick={() => setLocation("/ingest")}>
              Ingestion
            </button>{" "}
            to upload reviews. Sarcastic and ambiguous reviews will appear here for moderation.
          </p>
        </div>
      ) : (
        <>
          ...
          {stats.total === 0 && (
            <div className="py-8 text-center text-muted-foreground">
              ...
              <p className="text-sm mt-1">
                No sarcastic or ambiguous reviews found in the current dataset, or all have been reviewed.
              </p>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  Decisions persisted to localStorage
                </div>
              </div>
